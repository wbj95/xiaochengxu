...
//标题框初始化内容
const titletext_default = '您可以使用自然语言查询知识库'
...
Page({
  ...
  knowledge: function(corpus, object) {
    ...
    if (!isNaN(corpus)) {
      ...
      wx.request({
        url: 'http://120.26.174.202:8080/carmanage/CarInfo/findFilesByNum?fileName=' + wx.getStorageSync(corpus),
        method: 'GET',
        data: {},
        header: {
  ...
  findFile: function(keywords,text) {
    ...
    wx.request({
      url: 'http://120.26.174.202:8080/carmanage/CarInfo/findFiles',
      data: {
        array: keywords,
        cusid: userId
      },
      ...
      success: function(r) {
        ...
        if (r.data.files.length == 0) {
          ...
          var msg4 = '亲爱的主人，找到以下相似的答案，请选择第几项，直接回复数字或点击' + '\n'
          // var j = 0;
           var msglist = that.data.msglist;
          var msg2 = {
            'type': 2,
